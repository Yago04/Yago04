...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "yago-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">yago@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'yago@github:~$ whoami <tspan fill="{INK}">Yago</tspan></text>')
parts.append(f'<rect x="{PAD+175}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
